...
for row in cur.execute('SELECT id,element_category,model_name,manufacturer FROM element_models'):
    print(row)

print('ELEMENTS:')
for row in cur.execute('SELECT id,substation_id,element_type,name,serial_number,element_model_id,operating_status FROM elements'):
    print(row)
