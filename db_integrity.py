...
def check_database_integrity(db_path: str, quick_check: bool = False) -> IntegrityCheckResult:
    ...
    for table_name, required_columns in required_tables.items():
        ...
        for col in required_columns:
            if col not in existing_columns:
                result.add_error(f"Required column '{col}' is missing from table '{table_name}'")
                schema_valid = False
...
def _check_foreign_keys(cursor: sqlite3.Cursor, result: IntegrityCheckResult):
    ...
    if violations:
        ...
        if len(violations) > 10:
            result.add_error(f"... and {len(violations) - 10} more foreign key violations")
    else:
...
def _check_required_fields(cursor: sqlite3.Cursor, result: IntegrityCheckResult):
    ...
    for table, column, field_name in checks:
        try:
            ...
            if null_count > 0:
                result.add_warning(f"{null_count} records in '{table}' have NULL {field_name}")
            ...
            if column != "substation_id" and 'id' not in column:
                ...
                if empty_count > 0:
                    result.add_warning(f"{empty_count} records in '{table}' have empty {field_name}")
        
        except Exception as e:
            result.add_warning(f"Could not check required field '{column}' in '{table}': {e}")
...
def _check_data_consistency(cursor: sqlite3.Cursor, result: IntegrityCheckResult):
    ...
    try:
        ...
        if maint_no_elements > 0:
            result.add_warning(f"{maint_no_elements} maintenance records have no associated elements")
    except Exception:
        pass
...
def _check_orphaned_records(cursor: sqlite3.Cursor, result: IntegrityCheckResult):
    ...
    try:
        ...
        if orphaned_elements > 0:
            result.add_error(f"{orphaned_elements} elements reference non-existent substations")
    except Exception:
        pass
    ...
    try:
        ...
        if orphaned_maint_elem > 0:
            result.add_error(f"{orphaned_maint_elem} maintenance_elements reference non-existent maintenance")
    except Exception:
        pass
    ...
    try:
        ...
        if orphaned_maint_elem2 > 0:
            result.add_error(f"{orphaned_maint_elem2} maintenance_elements reference non-existent elements")
    except Exception:
        pass
