...
def test_ruff_checks():
    if not tool_available("ruff"):
        pytest.skip(
            "ruff not installed; install -r requirements-dev.txt to enable CI lint checks"
        )
...
def test_flake8_checks():
    if not tool_available("flake8"):
        pytest.skip(
            "flake8 not installed; install -r requirements-dev.txt to enable CI lint checks"
        )
    ...
    run_module(
        [
            "flake8",
            "--max-line-length=88",
            "--exclude=.venv,venv,scripts,.dist,dist,tests/_shims",
        ]
    )
