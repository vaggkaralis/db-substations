...
def test_android_change_import():
    ...
    try:
        ...
        cur.execute("INSERT INTO substations (name, location) VALUES (?, ?)", ("S1", "Loc1"))
        ...
        try:
            maintenance_payload = {
                ...
                "elements": [
                    {"element_id": elem_id, "element_comments": "Checked"}
                ],
            }
            with open(changelog, "w", encoding="utf-8") as fh:
                fh.write(json.dumps({"operation": "insert", "table": "maintenance", "data": maintenance_payload}, ensure_ascii=False) + "\n")

            # Apply change log
            apply_change_log_to_db(conn, changelog)

            # Verify maintenance inserted
            cur.execute("SELECT id, substation_id, date_time, overall_comments FROM maintenance WHERE substation_id=?", (sub_id,))
            rows = cur.fetchall()
            assert len(rows) == 1
            m = rows[0]
            ...
            cur.execute("SELECT maintenance_id, element_id, element_comments FROM maintenance_elements WHERE maintenance_id=?", (m[0],))
            elems = cur.fetchall()
            assert len(elems) == 1
