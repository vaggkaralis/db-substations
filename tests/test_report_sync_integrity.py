...
def test_report_tracking_row_removed_when_pair_deleted(tmp_path):
    ...
    conn.commit()
    assert get_maintenance_report_path(conn, maintenance_id=100, element_id=10, verify_exists=False)

    conn.execute("DELETE FROM maintenance_elements WHERE maintenance_id=? AND element_id=?", (100, 10))
    conn.commit()

    assert get_maintenance_report_path(conn, maintenance_id=100, element_id=10, verify_exists=False) is None
...
def test_ensure_maintenance_overview_reports_tracks_each_reports_root(tmp_path, monkeypatch):
    conn = init_db(str(tmp_path / 'test.db'))
    _seed_sample_data(conn)
    ...
    conn.execute(
        ...
        """,
        (
            100, 'gate:1', str(tmp_path / 'gate1'), str(tmp_path / 'gate1' / 'inst'), str(tmp_path / 'gate1' / 'media'), str(gate1_reports), 'now',
            100, 'gate:2', str(tmp_path / 'gate2'), str(tmp_path / 'gate2' / 'inst'), str(tmp_path / 'gate2' / 'media'), str(gate2_reports), 'now',
        ),
    )
    conn.commit()
    ...
    monkeypatch.setattr('pdf_reports.generate_maintenance_overview_report', fake_generate)

    result = ensure_maintenance_overview_reports(conn, maintenance_id=100, db_path=str(tmp_path / 'test.db'))

    assert result['errors'] == []
    assert result['generated'] == 2
    assert get_maintenance_overview_report_path(conn, maintenance_id=100, gate_key='gate:1', verify_exists=False)
    assert get_maintenance_overview_report_path(conn, maintenance_id=100, gate_key='gate:2', verify_exists=False)


def test_export_missing_reports_regenerates_missing_file_even_when_db_row_exists(tmp_path, monkeypatch):
    conn = init_db(str(tmp_path / 'test.db'))
    _seed_sample_data(conn)
    ...
    conn.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        (100, 'gate:1', str(tmp_path / 'gate1'), str(tmp_path / 'gate1' / 'inst'), str(tmp_path / 'gate1' / 'media'), str(tmp_path / 'gate1' / 'reports'), 'now'),
    )
    conn.execute(
    ...
    monkeypatch.setattr('report_sync.safe_generate_and_store_report', fake_safe_generate_and_store_report)
    monkeypatch.setattr('report_sync.ensure_maintenance_overview_reports', fake_ensure_overview_reports)
...
def test_export_missing_reports_backfills_overview_when_element_pdf_already_exists(tmp_path, monkeypatch):
    conn = init_db(str(tmp_path / 'test.db'))
    _seed_sample_data(conn)
    ...
    conn.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        (100, 'gate:1', str(tmp_path / 'gate1'), str(tmp_path / 'gate1' / 'inst'), str(tmp_path / 'gate1' / 'media'), str(reports_root), 'now'),
    )
    conn.execute(
        "INSERT INTO maintenance_report_paths (maintenance_id, element_id, report_type, report_path, created_at, updated_at) VALUES (?, ?, 'pdf', ?, 'now', 'now')",
    ...
    def fake_safe_generate_and_store_report(*args, **kwargs):
        raise AssertionError('Element report should not be regenerated when existing PDF is usable')
...
def test_ensure_maintenance_overview_reports_regenerates_invalid_existing_pdf(tmp_path, monkeypatch):
    conn = init_db(str(tmp_path / 'test.db'))
    _seed_sample_data(conn)
