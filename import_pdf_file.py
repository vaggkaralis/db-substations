"""
Import a single maintenance record from a .pdf file into SQLite.
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Import a maintenance record from a .pdf file."
    )
    parser.add_argument("--file", required=False, help="Path to .pdf file")
    parser.add_argument(
    ...
    if not file_path:
        if sys.stdin.isatty():
            print("Select .pdf file to import")
            try:
