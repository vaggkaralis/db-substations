...
def show_substation_selection_window_with_callback_delegate(
    app, parent_popup, all_substations, on_select, title=None
):
    if title is None:
        title = S["MESSAGES"].get("SELECT_SUBSTATION", "Επιλογή Υποσταθμού")
    return app._show_substation_selection_window_with_callback(
        parent_popup, all_substations, on_select, title=title
    )
...
def show_specific_substation_from_window_delegate(
    app, substation_name, selection_popup
):
    return app._show_specific_substation_from_window(substation_name, selection_popup)


def display_substations_delegate(
    app,
    filter_name=None,
    reuse_popup=None,
