...
class PeopleManager:
    ...
    def show_people_management(self, instance=None):
        popup = Popup(
            title=S["MESSAGES"].get("PEOPLE_BUTTON", "Διαχείριση Προσωπικού"),
            size_hint=(0.7, 0.8),
        )
        ...
        form_layout.add_widget(
            Label(text=S["MESSAGES"].get("SURNAME_LABEL", "Επώνυμο:"), size_hint_x=0.3)
        )
        surname_input = TextInput(multiline=False, size_hint_x=0.7)
        form_layout.add_widget(surname_input)

        form_layout.add_widget(
            Label(text=S["MESSAGES"].get("NAME_LABEL", "Όνομα:"), size_hint_x=0.3)
        )
        given_input = TextInput(multiline=False, size_hint_x=0.7)
        form_layout.add_widget(given_input)

        form_layout.add_widget(
            Label(text=S["MESSAGES"].get("ROLE_LABEL", "Ρόλος:"), size_hint_x=0.3)
        )
        ...
        form_layout.add_widget(
            Label(text=S["MESSAGES"].get("EMAIL_LABEL", "Email:"), size_hint_x=0.3)
        )
        ...
        receiver_checkbox = CheckBox(
            size_hint_x=0.1, color=self.app.theme.get("primary", (0.05, 0.18, 0.36, 1))
        )
        ...
        active_checkbox = CheckBox(
            size_hint_x=0.1,
            active=True,
            color=self.app.theme.get("primary", (0.05, 0.18, 0.36, 1)),
        )
        active_layout.add_widget(active_checkbox)
        active_layout.add_widget(
            Label(text=S["MESSAGES"].get("ACTIVE_LABEL", "Ενεργός"), size_hint_x=0.9)
        )
        ...
        def refresh_list():
            ...
            for cat, items in grouped.items():
                ...
                items_sorted = sorted(
                    items,
                    key=lambda r: (
                        _role_priority(r[2] if len(r) > 2 else None),
                        (r[1] or "").lower(),
                    ),
                )
                for (
                    ...
                    status = (
                        S["MESSAGES"].get("ACTIVE_LABEL", "Ενεργός")
                        if active
                        else S["MESSAGES"].get("INACTIVE_LABEL", "Ανενεργός")
                    )
                    email_text = email if email else S["MESSAGES"].get("DASH", "-")
                    receiver_text = (
                        S["BUTTONS"].get("YES", "Ναι")
                        if report_receiver
        ...
        def _show_people_io_popup(_instance=None):
            try:
                ...
                show_message_popup(
                    S["TITLES"]["ERROR"],
                    S["MESSAGES"].get(
                        "EXCEL_HELPERS_MISSING",
                        "Οι βοηθητικές συναρτήσεις Excel δεν είναι διαθέσιμες.",
                    ),
                )
                return
            ...
            io_popup = Popup(
                title=S["MESSAGES"].get(
                    "IMPORT_EXPORT_PEOPLE", "Εισαγωγή/Εξαγωγή Προσωπικού"
                ),
                size_hint=(0.5, 0.4),
            )
            ...
            def _import_people(_btn=None):
                try:
                    fp = ask_open_file(
                        title="Επιλέξτε αρχείο εισαγωγής (Excel)",
                ...
                try:
                    ...
                    show_message_popup(
                        S["TITLES"]["SUCCESS"], "Εισαγωγή προσωπικού ολοκληρώθηκε."
                    )
                    refresh_list()
                except Exception as exc:
                    show_message_popup(
                        S["TITLES"]["ERROR"],
                        f"Αποτυχία εισαγωγής προσωπικού:\n{str(exc)}",
                    )
            ...
            def _export_people(_btn=None):
                try:
                    ...
                    show_message_popup(
                        S["TITLES"]["ERROR"],
                        f"Αποτυχία εξαγωγής προσωπικού:\n{str(exc)}",
                    )
            ...
            def _export_template(_btn=None):
                try:
                    ...
                    show_message_popup(
                        S["TITLES"]["ERROR"],
                        f"Αποτυχία δημιουργίας προτύπου:\n{str(exc)}",
                    )

            imp_btn = Button(
                text=S["MESSAGES"].get(
                    "IMPORT_PEOPLE_BTN", "Εισαγωγή Προσωπικού (Excel)"
                )
            )
            ...
            exp_btn = Button(
                text=S["MESSAGES"].get(
                    "EXPORT_PEOPLE_BTN", "Εξαγωγή Προσωπικού (Excel)"
                )
            )
            ...
            tpl_btn = Button(
                text=S["MESSAGES"].get(
                    "EXPORT_PEOPLE_TEMPLATE_BTN", "Δημιουργία Template Προσωπικού"
                )
            )
    ...
    def _confirm_delete_person(self, person_id, person_name, refresh_cb=None):
        ...
        confirm_delete_person_message = confirm_delete_person_template.format(
    ...
    def _show_edit_person_popup(self, person_id, refresh_cb=None):
        ...
        def save_changes():
            ...
            if not new_surname or not new_role:
                show_message_popup(
                    "Σφάλμα", "Το επώνυμο και ο ρόλος είναι υποχρεωτικά!"
                )
                return
