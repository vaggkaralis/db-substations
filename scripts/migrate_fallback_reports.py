...
def build_target_path(conn, db_path, maintenance_id, element_row):
    ...
    if not targets:
        raise RuntimeError(f"No report targets found for M{maintenance_id}_E{element_id}")
...
def match_files_to_elements(maintenance_id, source_files, element_rows):
    ...
    exact_name_map = {
        canonical_report_name(row["substation_name"], row["element_name"], maintenance_id).lower(): row["element_id"]
    ...
    for source_file in source_files:
        ...
        elif lower_name in exact_name_map and exact_name_map[lower_name] in unmatched_elements:
...
def migrate_fallback_reports(*, conflict_policy: str = "skip"):
    ...
    for maintenance_id, maint_path in sorted(maintenance_dirs):
        try:
            ...
            if unmatched_elements:
                print(f"M{maintenance_id}: {len(unmatched_elements)} unmatched element(s)")
                for row in unmatched_elements[:5]:
                    print(f"  UNMATCHED ELEMENT: E{row['element_id']} {row['element_name']}")
            
            element_by_id = {row['element_id']: row for row in element_rows}
            for element_id, source_file in sorted(assignments.items()):
                try:
                    target_file = build_target_path(conn, db_path, maintenance_id, element_by_id[element_id])
                    ...
                    if os.path.normcase(source_abs) == os.path.normcase(target_abs):
                        ...
                        if filecmp.cmp(source_fs, target_fs, shallow=False):
                            ...
                            print(f"  M{maintenance_id}_E{element_id}: Removed duplicate fallback copy")
                        else:
                            if conflict_policy == "prefer-canonical":
                                stats['db_pointed_to_canonical_on_conflict'] += 1
                                print(f"  M{maintenance_id}_E{element_id}: Conflict at target, DB pointed to canonical")
                            else:
                                final_db_path = source_abs
                                print(f"  M{maintenance_id}_E{element_id}: Conflict at target, keeping fallback file")
                            stats['conflicts'] += 1
                    else:
                        os.makedirs(win_path(os.path.dirname(target_abs)), exist_ok=True)
                        shutil.move(source_fs, target_fs)
                        stats['moved'] += 1
                        print(f"  M{maintenance_id}_E{element_id}: Moved → {os.path.relpath(target_abs, shared_root)}")
                        final_db_path = target_abs
                    ...
                    print(f"  M{maintenance_id}_E{element_id}: Error moving report: {e}")
                    stats['errors'] += 1
    ...
    print(f"Conflicts pointed to canonical in DB: {stats['db_pointed_to_canonical_on_conflict']}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Migrate fallback reports into canonical paths")
    parser.add_argument(
        "--conflict-policy",
