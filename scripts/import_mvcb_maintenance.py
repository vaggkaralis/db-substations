...
from access_gate_utils import build_access_asset_gate_maps, find_mv_gate, format_gate_label
...
REPORT_PATH = SQLITE_PATH.parent / "reports" / "mvcb_access_import_report.json"
...
def load_access_models(cursor):
    rows = cursor.execute(
        "SELECT [MVCBModel ID], [Manufacturer], [Type] FROM tblMVCBModel"
    ).fetchall()
    models = {}
    for row in rows:
        models[int(row[0])] = {
...
def load_access_breaker_details(cursor, substations, models):
    rows = cursor.execute(
        "SELECT [MVCB ID], [Gate], [Μοντέλο], [MVCBName], [Serial No] FROM tblMVCBDetails"
    ).fetchall()
...
def load_access_maintenance_rows():
    with build_access_connection() as conn:
        ...
        rows = cursor.execute(
            "SELECT [ID], [Ημ/νία], [ΔΙ ΜΤ], [Συντηρητής], [Περιγραφή], [Ολοκληρώθηκε] "
            "FROM [tblMVCΒMaintenanceData] ORDER BY [ID]"
        ).fetchall()
...
def backup_sqlite_db():
    ...
    backup_path = BACKUP_DIR / f"substations_before_access_mvcb_import_{timestamp}.db"
...
def find_matching_element(maintenance_row, sqlite_breakers):
    ...
    access_manufacturer = normalize_text(maintenance_row.get("access_manufacturer"))
