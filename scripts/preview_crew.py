...
class CrewPreviewApp(App):
    def build(self):
        ...
        # header
        root.add_widget(Label(text='Maintenance Crew Preview', size_hint_y=None, height=30))
        ...
        for cat, members in grouped.items():
            ...
            cat_grid = GridLayout(cols=cols, spacing=(6,2), size_hint_y=None, padding=2)
            ...
            for pid, name, role in members:
                ...
                lbl = Label(text=f"{name} ({role})", halign='left', valign='middle', size_hint_x=1)
                lbl.size_hint_y = None
                lbl.height = min_cell_h
                def _update_label_height(inst, parent_cell=cell, anchor_container=anchor, checkbox=cb, name_val=name):
                    try:
                        ...
                        if os.environ.get('MAINT_DEBUG'):
                            print(f"MAINT_DEBUG: '{name_val}' -> cell_h={h}, label_w={inst.width}, checkbox_h={checkbox.height}")
                    except Exception:
                        pass
