...
COLUMN_MAPPINGS = {
    'Substation Name': ['Substation Name', 'Substation', 'Υποσταθμός', 'Όνομα Υποσταθμού', 'Station'],
    ...
    'Gate': ['Gate', 'Πύλη'],
    'Operating Status': ['Operating Status', 'Status', 'Κατάσταση', 'Κατάσταση Λειτουργίας'],
    'Maintenance Date': ['Maintenance Date', 'Date', 'Ημερομηνία', 'Ημερομηνία Συντήρησης', 'Maint Date'],
    'Model Name': ['Model Name', 'Model', 'Μοντέλο', 'Όνομα Μοντέλου'],
    'Model Manufacturer': ['Model Manufacturer', 'Model Mfg', 'Κατασκευαστής Μοντέλου', 'Manufacturer'],
    'Model Installation Space': ['Model Installation Space', 'Installation Space', 'Χώρος Εγκατάστασης', 'Space'],
    'Model Maintenance Cycle': ['Model Maintenance Cycle', 'Model Cycle', 'Κύκλος Μοντέλου'],
    'Breaker Role': ['Breaker Role', 'Role', 'Ρόλος', 'Ρόλος Διακόπτη'],
    'Τύπος Διακόπτη': ['Τύπος Διακόπτη', 'Breaker Type', 'Category', 'Κατηγορία', 'Breaker Category'],
}

# Expected values for element types
ELEMENT_TYPE_MAPPINGS = {
    'Μετασχηματιστής 150/20KV': ['Μετασχηματιστής 150/20KV', 'Μετασχηματιστής', 'Transformer 150/20', 'Transformer'],
    'Μετασχηματιστής 20/0.4KV': ['Μετασχηματιστής 20/0.4KV', 'Μετασχηματιστής 20/0.4', 'Transformer 20/0.4'],
    'Διακόπτης ΥΤ': ['Διακόπτης ΥΤ', 'Κεντρικός Διακόπτης ΥΤ', 'HV Breaker', 'HV Circuit Breaker'],
    'Διακόπτης ΜΤ': ['Διακόπτης ΜΤ', 'Κεντρικός Διακόπτης ΜΤ', 'Διακόπτης Φορτίου Γραμμής ΜΤ', 'MV Breaker', 'MV Circuit Breaker'],
...
OPERATING_STATUS_MAPPINGS = {
    ...
    'Ανενεργή': ['Ανενεργή', 'Inactive', 'Ανενεργός', 'Ανενεργό', 'Εκτός Λειτουργίας', 'Out of Service'],
}
...
BREAKER_ROLE_MAPPINGS = {
    ...
    'Διακόπτης Πυκνωτών': ['Διακόπτης Πυκνωτών', 'Capacitor', 'Capacitor Breaker', 'Cap Bank'],
}
...
def find_best_match(value: str, mapping_dict: Dict[str, List[str]], threshold: float = 0.6) -> Optional[Tuple[str, float, List[str]]]:
    """
...
def analyze_import_data(df, column_mapping: Dict, conn) -> Dict:
    ...
    # Cache models
    cursor.execute("SELECT element_category, model_name, manufacturer FROM element_models")
    ...
    REQUIRED_FIELDS = ['Substation Name', 'Element Type', 'Name', 'Serial Number', 'Gate', 'Operating Status']
    ...
    for idx, row in df.iterrows():
        ...
        if sub_name:
            if sub_name in db_substations:
                existing_substations[sub_name] = existing_substations.get(sub_name, 0) + 1
            else:
        ...
        if model_name and elem_type:
            ...
            if model_key in db_models:
                ...
                new_models_dict[model_key] = {'category': elem_type, 'name': model_name, 'manufacturer': model_manufacturer or ''}
