...
from ui.shared import IconWidget, ShiftSelectableTextInput, IconButton
...
class SubstationApp(App):
    ...
    def show_import_menu(self, instance):
        ...
        layout.add_widget(
            Label(text="Επιλέξτε τι θέλετε να εισάγετε:", size_hint_y=0.2)
        )

        # Import elements button
        import_elements_btn = Button(
            text="Εισαγωγή Στοιχείων από Αρχείο", size_hint_y=0.2
        )
        import_elements_btn.bind(
            on_press=lambda x: self._show_import_elements_from_menu(menu_popup)
        )
        layout.add_widget(import_elements_btn)

        # Import Android changes button
        import_android_btn = Button(
            text="Εισαγωγή αλλαγών από Android", size_hint_y=0.2
        )
        ...
        # Separator
        layout.add_widget(
            Label(text="Ή δημιουργήστε πρότυπο εισαγωγής:", size_hint_y=0.15)
        )

        # Template button
        template_elements_btn = Button(
            text="Δημιουργία Template Εισαγωγής", size_hint_y=0.2
        )
        ...
        if substations:
            for (
                ...
                header_layout.add_widget(
                    Label(text="Τοποθεσία", bold=True, size_hint_x=0.17)
                )
                header_layout.add_widget(
                    Label(text="Ανάληψη", bold=True, size_hint_x=0.1)
                )
                header_layout.add_widget(
                    Label(text="Στοιχεία", bold=True, size_hint_x=0.07)
                )
                header_layout.add_widget(
                    Label(text="Πύλες", bold=True, size_hint_x=0.07)
                )
                header_layout.add_widget(
                    Label(text="Πυκνωτές", bold=True, size_hint_x=0.07)
                )
                header_layout.add_widget(
                    Label(text="Συντηρήσεις", bold=True, size_hint_x=0.1)
                )
                header_layout.add_widget(
                    Label(text="Τελευταία", bold=True, size_hint_x=0.1)
                )
                header_layout.add_widget(
                    Label(text="Μονογραμμικό", bold=True, size_hint_x=0.12)
                )
                header_layout.add_widget(
                ...
                sub_row_layout.add_widget(
                    Label(text=adoption_date or "-", size_hint_x=0.1)
                )
                ...
                sub_row_layout.add_widget(
                    Label(text=str(capacitor_count), size_hint_x=0.07)
                )
                sub_row_layout.add_widget(Label(text=str(maint_count), size_hint_x=0.1))
                sub_row_layout.add_widget(
                    Label(text=last_maint_display, size_hint_x=0.1)
                )

                monogram_btn = Button(
                    text="Άνοιγμα" if monogram_pdf else "Προσθήκη", size_hint_x=0.12
                )
    ...
    def show_edit_element_popup(
        ...
        c = self.conn.cursor()
        c.execute(
            "SELECT element_type, name, serial_number, maintenance_date, voltage_level, manufacturer, model, model_version, installation_space, operating_status, maintenance_cycle, manufacture_year, element_model_id, gate, is_main_switch, breaker_category FROM elements WHERE id=?",
            (element_id,),
        )
        element = c.fetchone()
        ...
        breaker_category_options = self._get_breaker_categories_for_element_type(
            elem_type
        )
        ...
        def load_models_for_breaker_category(selected_category):
            ...
            models_data_temp, display_names, selected_display_name = (
                self._load_models_for_element_type(
    ...
    def show_add_element_popup(self, instance):
        ...
        initial_gates = self.get_available_gates(
            self.substations_map[substation_names[0]]
        )
        gate_spinner = Spinner(
        ...
        initial_breaker_categories = self._get_breaker_categories_for_element_type(
        ...
        def on_breaker_category_change(spinner, text):
            ...
            try:
                # load_models_for_category will be defined later in this scope
    ...
    def show_add_element_popup_for_substation(
        ...
        def load_models_for_category(category, selected_breaker_category=None):
            ...
            if models:
                ...
                if category in ["Διακόπτης ΜΤ", "Διακόπτης ΥΤ"]:
                    ...
                    model_spinner.text = (
                        display_names[0] if display_names else "Δεν υπάρχουν μοντέλα"
                    )
                else:
                    ...
                    model_spinner.values = display_names
                    model_spinner.text = (
                        display_names[0] if display_names else "Επιλέξτε μοντέλο"
                    )
            else:
                model_spinner.values = ["Επιλέξτε μοντέλο"]
                model_spinner.text = "Επιλέξτε μοντέλο"
        ...
        def on_model_selected(spinner, text):
            if text in models_data:
                ...
                if "maintenance_cycle" in field_inputs:
                    field_inputs["maintenance_cycle"].text = str(
                        model["maintenance_cycle"]
                    )
                if "installation_space" in field_inputs:
                    field_inputs["installation_space"].text = model[
                        "installation_space"
                    ]
        ...
        def add_element():
            ...
            show_message_popup(
                "Επιτυχία",
                "Στοιχείο προστέθηκε!",
                callback=lambda: self._display_substations(selected_substation_name),
            )
                ...
                for (
                    ...
                    def build_details_for(eid):
                        ...
                        # High-voltage SF6-specific layout (only for ΥΤ & SF6)
                            ...
                            if elem_type == "Μετασχηματιστής 150/20KV":
                                # Fetch stored power (MVA) for this element
                                try:
                                    c = self.conn.cursor()
                                    c.execute("SELECT power_mva FROM elements WHERE id=?", (eid,))
                                    row = c.fetchone()
                                    power_val = row[0] if row and row[0] is not None else 50
                                except Exception:
                                    power_val = 50

                                # Power field (ΙΣΧΥΣ)
                                power_row = BoxLayout(size_hint_y=None, height=30, spacing=6)
                                power_row.add_widget(Label(text="ΙΣΧΥΣ (MVA):", size_hint_x=0.4))
                                power_input = TextInput(text=str(power_val), multiline=False, size_hint_x=0.2)
                                power_row.add_widget(power_input)
                                power_row.add_widget(Widget())
                                details_container.add_widget(power_row)
                                ...
                                silica_spinner = Spinner(text="OK", values=["OK", "NOT OK", "N/A"], size_hint_x=0.2)
                                ...
                                measurements.setdefault("power_mva", power_input)
    ...
    def show_isolation_requests(self, instance=None):
        ...
        legend_layout.add_widget(
            Label(
                text="● Αιτήθηκε", size_hint_x=0.2, color=(1, 0.85, 0, 1), **font_kwargs
            )
        )
        legend_layout.add_widget(
            Label(
        ...
        def load_calendar():
            ...
            for day in range(1, days_in_month + 1):
                ...
                if day in requests_by_day:
                    ...
                    requests_layout.bind(
                        minimum_height=requests_layout.setter("height")
                    )
                    ...
                    for (
                        ...
                        def make_request_handler(r_id):
                            return lambda x: self.show_isolation_request_details(
                                r_id, popup
                            )
    ...
    def show_add_isolation_request(self, parent_popup):
        ...
        # Start datetime
        layout.add_widget(
            Label(text="Ημ/νία & Ώρα Έναρξης:", size_hint_y=None, height=30)
        )
        ...
        # End datetime
        layout.add_widget(
            Label(text="Ημ/νία & Ώρα Λήξης:", size_hint_y=None, height=30)
        )
        end_input = TextInput(
        ...
        def save_request():
            ...
            try:
                ...
                if end <= start:
                    show_message_popup(
                        "Σφάλμα", "Η ημερομηνία λήξης πρέπει να είναι μετά την έναρξη!"
                    )
                    return
            except ValueError:
                show_message_popup(
    ...
    def show_isolation_request_details(self, request_id, parent_popup):
        ...
        # Start datetime
        details_layout.add_widget(
            Label(text="Έναρξη:", size_hint_y=None, height=30, bold=True)
        )
        start_input = TextInput(
            text=start_dt, size_hint_y=None, height=35, multiline=False
        )
        details_layout.add_widget(start_input)

        # End datetime
        details_layout.add_widget(
            Label(text="Λήξη:", size_hint_y=None, height=30, bold=True)
        )
        ...
        # Status
        details_layout.add_widget(
            Label(text="Κατάσταση:", size_hint_y=None, height=30, bold=True)
        )
        status_spinner = Spinner(
            text=status,
            values=["Requested", "Accepted", "Cancelled"],
        ...
        # Notes
        details_layout.add_widget(
            Label(text="Σημειώσεις:", size_hint_y=None, height=30, bold=True)
        )
        notes_input = TextInput(
            text=notes or "", size_hint_y=None, height=80, multiline=True
        )
