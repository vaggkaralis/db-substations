...
class ColumnMappingPopup:
        ...
        if sub_col and sub_col in self.df.columns:
            ...
            for _, row in self.df.iterrows():
                ...
                if pd.notna(sub_name) and str(sub_name).strip():
                    ...
                    cursor.execute('SELECT id FROM substations WHERE name=?', (sub_name_clean,))
                    if cursor.fetchone():
    ...
    def show(self):
        ...
        if self.substations_info['all']:
            sub_info_layout = BoxLayout(orientation='vertical', size_hint_y=None, spacing=5, padding=10)
            ...
            if self.substations_info['new']:
                ...
                with sub_info_layout.canvas.before:
                    ...
                    sub_info_layout.bg_rect = Rectangle(size=sub_info_layout.size, pos=sub_info_layout.pos)
                sub_info_layout.bind(size=lambda obj, _: setattr(obj.bg_rect, 'size', obj.size))
                sub_info_layout.bind(pos=lambda obj, _: setattr(obj.bg_rect, 'pos', obj.pos))
                
                sub_text = 'ΥΠΟΣΤΑΘΜΟΙ ΠΡΟΣ ΔΗΜΙΟΥΡΓΙΑ:\n' + ', '.join(self.substations_info['new'])
                if self.substations_info['existing']:
                    sub_text += '\n\nΥΠΑΡΧΟΝΤΕΣ: ' + ', '.join(self.substations_info['existing'])
            ...
            sub_label.bind(
                ...
                texture_size=lambda obj, _: setattr(obj, 'height', obj.texture_size[1] + 10)
            )
            sub_info_layout.add_widget(sub_label)
            main_layout.add_widget(sub_info_layout)
        
        # Header info
        if self.mismatch_info['unmatched_import'] or self.mismatch_info['unmatched_required']:
            info_layout = BoxLayout(orientation='vertical', size_hint_y=None, height=80, spacing=5)
            
            # Create colored background
            with info_layout.canvas.before:
                Color(1, 0.95, 0.7, 1)  # Light yellow background
                info_layout.bg_rect = Rectangle(size=info_layout.size, pos=info_layout.pos)
            ...
            success_layout = BoxLayout(orientation='vertical', size_hint_y=None, height=60, spacing=5)
            
            with success_layout.canvas.before:
                Color(0.7, 1, 0.7, 1)  # Light green background
                success_layout.bg_rect = Rectangle(size=success_layout.size, pos=success_layout.pos)
            success_layout.bind(size=lambda obj, _: setattr(obj.bg_rect, 'size', obj.size))
        ...
        def get_available_options():
            ...
            available = ['-- Παράλειψη --'] + [col for col in COLUMN_MAPPINGS.keys() if col not in assigned]
            return available
        ...
        for import_col in self.df_columns:
            ...
            if import_col_clean in self.column_mapping:
                ...
                suggestions = self.mismatch_info['suggestions'].get(import_col_clean, [])
                ...
                if suggestions:
                    ...
                    if suggested_value in available_opts:
                        spinner = Spinner(
                            ...
                            size_hint_y=None,
                            height=40,
                            background_color=(1, 1, 0.7, 1)  # Light yellow for suggested
                        )
        ...
        if self.mismatch_info['unmatched_required']:
            warning_layout = BoxLayout(orientation='vertical', size_hint_y=None, height=100, spacing=5)
            
            with warning_layout.canvas.before:
                Color(1, 0.8, 0.8, 1)  # Light red background
                warning_layout.bg_rect = Rectangle(size=warning_layout.size, pos=warning_layout.pos)
            warning_layout.bind(size=lambda obj, _: setattr(obj.bg_rect, 'size', obj.size))
    ...
    def _create_spinner_callback(self, import_col):
        ...
        def callback(spinner, text):
            old_value = self.column_mapping.get(import_col)
    ...
    def _update_continue_button(self):
        ...
        required_set = set(COLUMN_MAPPINGS.keys())
        
        # At minimum, we need these critical columns
        critical_columns = {'Substation Name', 'Element Type', 'Name', 'Operating Status'}
    ...
    def _on_continue_press(self, instance):
        ...
        final_mapping = {k: v for k, v in self.column_mapping.items() if v != '-- Παράλειψη --'}
        self.popup.dismiss()
...
class DataValidationPopup:
    ...
    def show(self):
        ...
        if issue_count == 0:
            ...
            with summary_layout.canvas.before:
                Color(0.7, 1, 0.7, 1)
                summary_layout.bg_rect = Rectangle(size=summary_layout.size, pos=summary_layout.pos)
            summary_layout.bind(size=lambda obj, _: setattr(obj.bg_rect, 'size', obj.size))
        ...
        if new_sub_count > 0 or new_model_count > 0:
            info_layout = BoxLayout(orientation='vertical', size_hint_y=None, spacing=3, padding=5)
            ...
            if new_sub_count > 0:
                new_subs_text = f"ΝΕΟΙ ΥΠΟΣΤΑΘΜΟΙ ({new_sub_count}): " + ', '.join(self.analysis['new_substations'][:5])
                if new_sub_count > 5:
                ...
                new_subs_label.bind(
                    size=lambda obj, _: setattr(obj, 'text_size', (obj.width - 10, None)),
                    texture_size=lambda obj, _: setattr(obj, 'height', obj.texture_size[1] + 5)
                )
                info_layout.add_widget(new_subs_label)
            
            if new_model_count > 0:
                models_text = f"ΝΕΑ ΜΟΝΤΕΛΑ ({new_model_count}): "
                models_list = [f"{m['name']} ({m['category']})" for m in self.analysis['new_models'][:3]]
        ...
        if self.analysis['issues']:
            ...
            for row_num in sorted(issues_by_row.keys()):
                ...
                # Row header
                row_header = BoxLayout(orientation='vertical', size_hint_y=None, spacing=5)
                row_header.bind(minimum_height=row_header.setter('height'))
                
                with row_header.canvas.before:
                    Color(0.95, 0.95, 0.95, 1)
                    row_header.bg_rect = Rectangle(size=row_header.size, pos=row_header.pos)
                row_header.bind(size=lambda obj, _: setattr(obj.bg_rect, 'size', obj.size))
            ...
            with row_num_header.canvas.before:
                Color(0.2, 0.2, 0.5, 1)
                row_num_header.bg_rect = Rectangle(size=row_num_header.size, pos=row_num_header.pos)
            row_num_header.bind(size=lambda obj, _: setattr(obj.bg_rect, 'size', obj.size))
            ...
            for idx, row in self.df.iterrows():
                ...
                with row_num_cell.canvas.before:
                    Color(*row_color)
                    row_num_cell.bg_rect = Rectangle(size=row_num_cell.size, pos=row_num_cell.pos)
                row_num_cell.bind(size=lambda obj, _: setattr(obj.bg_rect, 'size', obj.size))
                row_num_cell.bind(pos=lambda obj, _: setattr(obj.bg_rect, 'pos', obj.pos))
                ...
                for canonical_col in columns:
                    ...
                    cell.bind(size=lambda obj, _: setattr(obj.bg_rect, 'size', obj.size))
        ...
        import_btn = Button(text='>> Εισαγωγή', bold=True, background_color=(0.3, 0.7, 0.3, 1))
    ...
    def _create_issue_widget(self, issue):
        ...
        issue_layout = BoxLayout(orientation='vertical', size_hint_y=None, spacing=5, padding=(10, 5))
