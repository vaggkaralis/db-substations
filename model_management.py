...
def show_add_model_popup(app_instance, parent_popup=None, category=None, callback=None):
    ...
    model_name_input = TextInput(hint_text='Όνομα Μοντέλου', size_hint_y=None, height=40, multiline=False)
    ...
    # Maintenance cycle
    layout.add_widget(Label(text='Κύκλος Συντήρησης (έτη):', size_hint_y=None, height=30))
    cycle_input = TextInput(hint_text='Αριθμός ετών', size_hint_y=None, height=40, multiline=False)
    ...
    def save_model():
        ...
        try:
            ...
            if callback:
                show_message_popup('Επιτυχία', 'Το μοντέλο προστέθηκε!', callback=callback)
            elif parent_popup:
                parent_popup.dismiss()
                show_message_popup('Επιτυχία', 'Το μοντέλο προστέθηκε!', callback=lambda: show_models_management(app_instance))
            else:
...
def show_edit_model_popup(app_instance, model_id, parent_popup):
    ...
    layout.add_widget(Label(text=f'Κατηγορία: {category}', size_hint_y=None, height=30, bold=True))
    
    # Model name
    layout.add_widget(Label(text='Όνομα Μοντέλου:', size_hint_y=None, height=30))
    model_name_input = TextInput(text=model_name, size_hint_y=None, height=40, multiline=False)
    layout.add_widget(model_name_input)
    
    # Manufacturer
    layout.add_widget(Label(text='Κατασκευαστής:', size_hint_y=None, height=30))
    manufacturer_input = TextInput(text=manufacturer or '', size_hint_y=None, height=40, multiline=False)
    layout.add_widget(manufacturer_input)
    
    # Maintenance cycle
    layout.add_widget(Label(text='Κύκλος Συντήρησης (έτη):', size_hint_y=None, height=30))
    cycle_input = TextInput(text=str(cycle), size_hint_y=None, height=40, multiline=False)
    layout.add_widget(cycle_input)
    ...
    def save_changes():
        ...
        c.execute(
            ...
            (new_model_display, manufacturer_input.text.strip(), cycle_val, space_spinner.text, model_id)
        )
        
        app_instance.conn.commit()
        popup.dismiss()
        parent_popup.dismiss()
        show_message_popup('Επιτυχία', 'Το μοντέλο και όλα τα συνδεδεμένα στοιχεία ενημερώθηκαν!', callback=lambda: show_models_management(app_instance))
...
def show_model_usages(app_instance, model_id, model_name):
    ...
    if usages:
        ...
        for elem_data in usages:
            elem_id, elem_type, elem_name, serial_number, maintenance_date, manufacturer, installation_space, operating_status, maintenance_cycle, breaker_category, manufacture_year, substation_name, substation_id = elem_data
            ...
            if current_substation != substation_name:
                ...
                substation_header_layout = BoxLayout(size_hint_y=None, height=40, spacing=10)
                ...
                jump_btn = Button(
                    text='Μετάβαση στον Υποσταθμό',
                    size_hint_x=0.3
                )
                jump_btn.bind(on_press=lambda x, sname=substation_name, p=popup: jump_to_substation(app_instance, sname, p))
                substation_header_layout.add_widget(jump_btn)
                
                grid.add_widget(substation_header_layout)
            
            # Element details box
            elem_box = BoxLayout(size_hint_y=None, height=90, spacing=5, orientation='vertical', padding=(10, 0, 0, 0))
            
            # Element name and type (bold, larger)
            breaker_info = f" | {breaker_category}" if breaker_category else ""
            name_text = f'[b][size=16]{elem_name}[/size][/b] - {elem_type}{breaker_info}'
            name_label = Label(
                text=name_text,
                size_hint_y=None,
                height=25,
        ...
        no_usage_label = Label(
            text='Το μοντέλο δεν χρησιμοποιείται σε κανένα στοιχείο.',
            size_hint_y=0.7
        )
