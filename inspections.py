...
def import_inspections_from_file(app, file_path):
    ...
    try:
        if file_path.endswith(".xlsx"):
            ...
            show_message_popup(
                S["TITLES"]["ERROR"], S["MESSAGES"]["UNSUPPORTED_FILE_FORMAT"]
            )
            return
    ...
    for _, row in df.iterrows():
        ...
        inspection_date = _parse_inspection_date(date_value) or datetime.now().strftime(
            "%Y-%m-%d"
        )
    ...
    show_message_popup(
        ...
        S["MESSAGES"]
        .get(
            "IMPORT_INSPECTIONS_DONE", f"Ολοκληρώθηκε η εισαγωγή ({inserted} εγγραφές)."
        )
...
def _show_edit_inspection_popup(app, inspection_id, fields):
    ...
    try:
        ...
        popup = Popup(
            title=S["TITLES"].get("INSPECTION_DETAILS", "Επεξεργασία Επιθεώρησης"),
            size_hint=(0.95, 0.9),
        )
        ...
        content = GridLayout(
            cols=1, spacing=4, size_hint_y=None, size_hint_x=1, padding=4
        )
        ...
        def _add_section(title, items):
            ...
            for it in items:
                ...
                lbl.bind(
                    size=lambda inst, sz: setattr(inst, "text_size", (inst.width, None))
                )
                row.add_widget(lbl)
                row.add_widget(ti)
        ...
        meta_block = [
            {"label": f.get("label"), "value": f.get("value")} for f in meta_items
        ]
        _add_section(
            S["MESSAGES"].get("INSPECTION_SECTION_META", "Μεταδεδομένα"), meta_block
        )
        ...
        for sec_title, start, end in sections:
            ...
            for i in range(start, min(end, len(rows))):
                if idx >= len(body):
                    ...
                    sec_items.append(
                        {"label": rows[i], "value": body[idx].get("value")}
                    )
                idx += 1
        ...
        if idx < len(body):
            rest_items = [
                {"label": f.get("label"), "value": f.get("value")} for f in body[idx:]
            ]
            _add_section(
                S["MESSAGES"].get("INSPECTION_SECTION_7", "Απόψεις"), rest_items
            )
        ...
        def _save(_):
            ...
            try:
                ...
                if substation_name:
                    c.execute(
                        "SELECT id FROM substations WHERE name=?", (substation_name,)
                    )
                    r = c.fetchone()
            ...
            try:
                ...
                show_message_popup(
                    ...
                    S["MESSAGES"].get("INSPECTION_SAVED", "Η επιθεώρηση καταχωρήθηκε!"),
                )
            except Exception:
                pass
...
def handle_inspection_menu(app, instance=None):
    ...
    try:
        ...
        entry_btn = Button(
            text=S.get("TITLES", {}).get("INSPECTION_ENTRY", "Καταχώρηση Επιθεώρησης")
        )
        hist_btn = Button(
            text=S.get("TITLES", {}).get("INSPECTION_HISTORY", "Ιστορικό Επιθεώρησης")
        )
        ...
        close_btn = Button(
            text=S.get("BUTTONS", {}).get("CANCEL", "Ακύρωση"), size_hint_y=0.3
        )
    ...
    try:
        ...
        if count == 0:
            ...
            show_message_popup(
                S["TITLES"].get("INSPECTION_HISTORY", "Ιστορικό Επιθεώρησης"),
                S["MESSAGES"]
                .get("INSPECTION_COUNT_FMT", "{count} εγγραφές επιθεώρησης")
                .format(count=count),
            )
    except Exception:
...
def handle_substation_inspection_history(
    app, substation_id, substation_name, parent_display_popup=None
):
    ...
    if hasattr(app, "_show_substation_inspection_history"):
        return getattr(app, "_show_substation_inspection_history")(
    ...
    try:
        c = app.conn.cursor()
        c.execute(
            "SELECT COUNT(*) FROM inspections WHERE substation_id=?", (substation_id,)
        )
        row = c.fetchone()
        count = row[0] if row else 0
        from popups import show_message_popup
        ...
        show_message_popup(
            ...
            .get(
                ...
                "Ιστορικό Επιθεώσεων - {substation_name}",
            )
            .format(substation_name=substation_name),
            S["MESSAGES"]
            .get(
                "SUBSTATION_INSPECTION_COUNT_FMT",
                "{count} εγγραφές επιθεώρησης για τον υποσταθμό {substation_name}",
            )
            .format(count=count, substation_name=substation_name),
        )
    except Exception:
        return None
...
def handle_inspection_details(app, inspection_id):
    ...
    try:
        c = app.conn.cursor()
        c.execute(
            "SELECT substation_name, inspection_date, data_json FROM inspections WHERE id=?",
            (inspection_id,),
        )
        row = c.fetchone()
        if not row:
            ...
            show_message_popup(
                ...
                S["MESSAGES"].get("RECORD_NOT_FOUND", "Το αρχείο δεν βρέθηκε"),
            )
            return None
        ...
        try:
            ...
            popup = Popup(
                title=S["TITLES"].get("INSPECTION_DETAILS", "Λεπτομέρειες Επιθεώρησης"),
                size_hint=(0.98, 0.95),
            )
            layout = BoxLayout(orientation="vertical", spacing=8, padding=8)
            ...
            meta = BoxLayout(
                orientation="horizontal", size_hint_y=None, height=60, spacing=8
            )
            meta_left = BoxLayout(orientation="vertical")
            meta_left.add_widget(
                Label(
                    text=f"[b]{S['MESSAGES'].get('SUBSTATION_LABEL_PLAIN', 'Υποσταθμός')}:[/b] {sub_name or '-'}",
                    markup=True,
                )
            )
            meta_left.add_widget(
                Label(
                    text=f"[b]{S['MESSAGES'].get('DATE_PLAIN', 'Ημερομηνία')}:[/b] {insp_date}",
                    markup=True,
                )
            )
            ...
            content = GridLayout(
                cols=1, spacing=0, size_hint_y=None, size_hint_x=1, padding=4
            )
            content.bind(minimum_height=content.setter("height"))
            ...
            def _make_section(title, items, default_expanded=False):
                ...
                body_grid = BoxLayout(
                    orientation="vertical", spacing=0, size_hint_y=None, size_hint_x=1
                )
            ...
            if idx < len(body):
                ...
                content.add_widget(
                    _make_section(
                        S["MESSAGES"].get("INSPECTION_SECTION_7", "Απόψεις"),
                        rest_items,
                        default_expanded=True,
                    )
                )
...
def show_substation_inspection_history_delegate(
    app, substation_id, substation_name, parent_display_popup=None
):
    return getattr(app, "show_substation_inspection_history")(
        substation_id, substation_name, parent_display_popup
    )
